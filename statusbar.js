...
const SB_CARD_CAT_SEED = ["心情","想法","天气","季节","着装","位置","现居地","格言"];
  ...
  root.innerHTML = `
    ...
    <div class="sb-card">
      ...
      <div class="sb-editor-actions">
        ...
        <button class="pill-btn ghost" onclick="window.sbResetTemplate('I')">恢复模板 Ⅰ</button>
        <button class="pill-btn ghost" onclick="window.sbResetTemplate('II')">恢复模板 Ⅱ</button>
      </div>
    </div>
...
window.sbResetTemplate = function(which){
  if(!confirm(`恢复内置模板 ${which==="I"?"Ⅰ":"Ⅱ"}？当前 HTML 将被覆盖。`)) return;
  const preset = sbActivePreset(sbSettingsSide);
  preset.html = which==="I" ? SB_BUILTIN_HTML_I : SB_BUILTIN_HTML_II;
  preset.builtin = which;
  sbSave(); window.renderStatusbarSettings();
};
...
// ─── 字卡库：状态栏专属标签栏 ───
function sbAllCats(){ return [...new Set([...SB_CARD_CAT_SEED, ...sbCards.map(c=>c.cat)])]; }
window.renderSbCards = function(){
  ...
  const m = {};
  SB_CARD_CAT_SEED.forEach(cat=>{ m[cat]=[]; });
  ...
  catList.forEach(cat=>{
    ...
    const isSeedEmpty = !m[cat].length;
    if(q && !filtered.length) return; // 搜索时隐藏完全不匹配的分类
    if(!q && isSeedEmpty && !SB_CARD_CAT_SEED.includes(cat)) return; // 非种子的空分类不展示
    shown++;
    ...
    let h = `<div class="cat-h" data-cat="${escapeHtml(cat)}">
      ...
      <div class="ops" onclick="event.stopPropagation()">${SB_CARD_CAT_SEED.includes(cat)?"":`<span class="danger" onclick="window.sbDeleteCat('${escapeAttr(cat)}')">删</span>`}</div>
    </div><div class="cat-body">`;
    if(!filtered.length){
      h += `<div class="sb-empty-tip" style="padding:14px 0;">这个分类还没有字卡，点右上角「＋」添加一张试试</div>`;
    }
    filtered.forEach(c=>{
